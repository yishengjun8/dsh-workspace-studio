 ...
 * ~/.dsh-plugin/dsh-workspace-studio/mindmap/. It is a flat list of SESSIONS
 * (horizontal chains of question cards) hanging off the virtual root or a
...
/* Docs carry every turn's question (answers are not persisted), so they exceed the generic 4 KiB mutation cap; give them a 2 MiB bound. */
export const MINDMAP_DOC_MAX_BYTES = 2 * 1024 * 1024
/* In-memory sync cache keyed by the ROOT session id: a cheap signature serves the cached doc when nothing changed, avoiding a full re-parse each 2.5 s poll; the TTL forces a periodic full sync so in-place log edits are never missed. */
...
/* Serialized on purpose: concurrent calls to the same provider interfere, so the queue runs ONE worker; the pump still drains the queue, just never in parallel. */
const MINDMAP_SUMMARY_CONCURRENCY = 1
const MINDMAP_SUMMARY_ENQUEUE_PER_SYNC = 5
/* One-time cap for a "summarize this session" request's missing-card backfill: a single long session must not fire hundreds of LLM calls at once; remaining missing turns are enqueued by the routine per-sync scan while the session summary stays pending. */
...
/* Keys whose LLM call the pump has actually STARTED (a subset of inFlight): a force='all' replacement for a running turn must wait for the original to finish, and inFlight alone cannot distinguish queued from running. Concurrency is 1, so this set holds at most one key. */
...
/* Shared in-flight LLM-call counter for BOTH the card-summary pump and the session-summary drain: parallel calls on the same provider interfere, so the two paths gate on ONE number — a card job blocks a session job until it finishes and vice versa. */
let mindmapSummaryWorkers = 0
/* Keys force-enqueued by the toolbar "重新生成全部摘要" action: these turns already carry a (stale) summary, so the has-summary skip in mindmapSummarizingOf is bypassed for them. */
const mindmapSummaryRegenerating = new Set()
/* Session-summary bookkeeping keys: `${rootId}\u0001${sessionId}`. The \u0001 separator is unambiguous — session ids exclude control characters, so a ':' inside a session id can never corrupt the split. */
...
/* Sessions waiting for a "总结当前会话" run: key → the config snapshot captured at request time. A session is ready when all its turns carry a summary and none is in-flight/regenerating. */
...
/* Per-ROOT enabled set: the old single global flag let the LAST sync's config govern every doc's queue, so one map's disable would silently stop another's. `mindmapSummaryFeatureOn` stays as a cheap "any root enabled" fast path derived from this set. */
const mindmapSummaryEnabledRoots = new Set()
let mindmapSummaryFeatureOn = false
let mindmapSummaryLastConfig = null

/* Serialize every mind-map doc mutation per ROOT session: sync and client POSTs would otherwise interleave their read-modify-write, letting a stale sync overwrite a just-written doc. The same lock covers rename, delete and the GET reconcile-and-write path; callers re-read the doc INSIDE the lock so a mutation landed between probe and lock is never clobbered. */
...
/* Re-anchor retry: when a root replacement lands between an operation's probe and its in-lock re-read, the live doc now lives under a DIFFERENT root. Re-acquiring the held root's lock self-deadlocks (ABBA), so the attempt throws this sentinel; the lock entry settles, releasing the old root, and the wrapper retries against the new root — each attempt holds exactly ONE lock, so no cycle is possible. */
...
/* Probe → per-root lock → in-lock re-read → op, with automatic re-anchor retry. `probe` resolves the current doc and picks the lock key; `reRead(root)` is the in-lock read; `op(doc)` runs under the lock. Returns op's result, or null when no valid doc resolves or the root keeps flipping past the retry budget. */
...
/* A doc on disk is a v3 document: { version: 3, rootSessionId, sessions }. Exported for the GET load path's degraded-refresh disk fallback. */
...
/* Every text block of a content list (reasoning skipped) joined with line breaks; the question may carry multiple blocks. */
...
/* Reverse-parse the FULL event log into completed turns { t, seq, user } from the direct human message (source.kind === 'user'). Answers are deliberately NOT stored; compaction copies (surfaceOp != append) are skipped so the human transcript, not the model-only surface, is shown. */
...
/* The LAST in-flight (unended) turn of a session's full log — the live card while the assistant generates. Scans BACKWARD from the tail: the first turn/end means the last turn closed, the first turn/start means it is still open — so an idle log costs O(1) and a streaming log only walks the open turn's own events. */
function mindmapLiveTurnOf(events) {
  ...
  for (let i = events.length - 1; i >= 0; i -= 1) {
    ...
    /* The last turn/start with no closer after it: collect its user messages (the same filtering as parseMindmapTurns). */
    let question = ''
...
/* Merge freshly parsed turns into a persisted list: existing turns match by owner-session turn/end seq and keep their display number; new turns get fresh numbers from the doc-wide counter. */
function reconcileMindmapTurns(parsed, existing, next) {
  ...
  for (const p of parsed) {
    ...
    if (old !== undefined) {
      /* Carry the persisted AI summary across the rebuild (same rule as `n`): the log has no summary — it is derived data owned by the doc. */
      const merged = {
        n: old.n,
        t: p.t,
        seq: p.seq,
        /* A compaction that REWROTE or dropped the original user/message event yields an empty parse for an already-recorded turn; keep the recorded text instead of blanking or re-concatenating the card. */
        user: p.user === '' ? old.user : p.user,
      }
      if (typeof old.summary === 'string' && old.summary !== '') merged.summary = old.summary
      /* Carry the persisted folded flag across the rebuild (same rule as `n` and `summary`): the log has no fold state — it is view state owned by the doc, so a sync must never silently unfold a card. */
...
/* Client-side summary config, validated leniently (a bad config is treated as "feature off"). Shapes: null/undefined -> off; { mode:'session', ... } -> each turn's owner-session model; { provider, model, ... } -> fixed route. length/sessionLength are ADVISORY (prompt wording only), clamped into bounds. */
...
/* The model route of one session for "follow the session model" mode: the live request header first, then the latest request/header event in the full log. Null when the session has no usable route — generation is skipped and the card keeps the original text. */
...
/* Scan a doc for turns without a summary and enqueue generation, bounded per call and deduped globally (in-flight and cooldown keys are skipped). Pure bookkeeping; workers run asynchronously and never block the sync response. `force` modes: undefined -> normal backfill; 'all' -> toolbar 重新生成全部摘要, bypassing both checks; 'missing' -> 总结当前会话 prerequisite, bypassing only the cooldown. In-flight keys are skipped except for force='all'. `onlySessionId` restricts the scan to ONE session's turns so the 总结当前会话 prerequisite never force-enqueues the whole doc. */
function mindmapEnqueueSummaries(ctx, persistence, doc, config, limit, force, onlySessionId) {
  if (config === null || config === undefined) {
    /* Feature turned off for THIS doc: drop its queued jobs so disabling stops token spend immediately; an in-flight call finishes but skips its write. Pending SESSION summaries of this doc are dropped too. Other docs' queues are untouched. */
  ...
  for (const session of doc.sessions ?? []) {
    ...
    for (const turn of session.turns ?? []) {
      ...
      if (mindmapSummaryInFlight.has(key)) {
        /* A turn whose previous job is still running cannot be re-enqueued with a changed config — EXCEPT for force='all', which queues a replacement the pump only starts AFTER the in-flight job, so the newer config's write lands last. Skip when a queued replacement already exists (double-press). */
...
/* Worker pump: at most MINDMAP_SUMMARY_CONCURRENCY in-flight calls; a finished job always pumps again so the queue drains without a timer. A queued job whose key is RUNNING (a force='all' replacement) waits so its write lands after the original. */
function mindmapSummaryPump() {
  while (mindmapSummaryWorkers < MINDMAP_SUMMARY_CONCURRENCY && mindmapSummaryQueue.length > 0) {
    ...
    /* The enqueue already added the key to inFlight; mark it RUNNING here and re-assert the inFlight + regenerating flags for a force='all' replacement — the original's finally cleared all three, and a replacement must keep the "generating summary" status visible until ITS summary lands. */
    ...
    void (async () => {
      try {
        ...
        mindmapSummaryPump()
        /* A finished card job may have made a pending session summary ready (its last missing/regenerating turn just landed). */
...
async function mindmapRunSummaryJob(job) {
  /* The job's OWN root must still have the feature on: the global flag only says "some doc is enabled" — another doc's disable must not stop this doc's queued work. */
  if (!mindmapSummaryFeatureOn || !mindmapSummaryEnabledRoots.has(String(job.rootId))) return
  const key = `${job.sessionId}:${job.seq}`
  /* Prefer the config snapshot captured when the job was enqueued: it belongs to the doc that enqueued it, while the global last-config may belong to a DIFFERENT doc's sync. The global is only a defensive fallback for jobs without a snapshot. */
...
/* The doc-family turns currently generating a summary (queued or running), for the client's per-card "正在生成摘要中…" status row. In-flight keys whose turn already carries a summary are skipped, and the list is sorted so the client's identity comparison stays stable. */
export function mindmapSummarizingOf(doc) {
  ...
  for (const key of mindmapSummaryInFlight) {
    /* lastIndexOf: a session id may itself contain ':' (the seq suffix is the final numeric segment), so a plain indexOf would split it wrongly. */
    ...
    /* Skip a turn that already has its summary: the write landed but the job's cleanup has not run yet. EXCEPT for force-enqueued regenerations, where the existing summary is the STALE one being replaced, so the status row must keep showing "正在生成摘要中…" until the new summary lands. */
...
/* B1 wrapper for the summary source: the text rides inside a CDATA-escaped <content_to_summarize> container so the model treats everything inside as the OBJECT of the summary — never as instructions. `]]>` is escaped the standard CDATA way, so the source text can never close the container early. */
function mindmapSummaryContent(text) {
  const safe = String(text ?? '').replace(/\]\]>/g, ']]]]><![CDATA[>')
  return `<content_to_summarize>\n<![CDATA[\n${safe}\n]]>\n</content_to_summarize>`
}

/* Consume one llm.stream under a hard deadline: a stuck for-await would never release the single summary worker slot, stalling the whole queue until a dsh restart. Racing consumption against a deadline slightly beyond the abort timer frees the worker even when the stream never settles. Returns the accumulated text, or null when the stream errored/aborted or the deadline fired first. */
...
/* Drain every pending session summary whose card summaries are now ready. Runs after each card-summary job and on each sync; the LLM call is background (never blocks sync). Failures cool down and drop the pending entry. */
export function mindmapDrainPendingSessionSummaries(ctx, persistence) {
  ...
  for (const [key, config] of [...mindmapSessionSummaryPending]) {
    ...
    /* Feature off for THIS root: drop its pending entry instead of draining it (the jobs re-check the flag too, but clearing here stops the loop). */
    if (!mindmapSummaryFeatureOn || !mindmapSummaryEnabledRoots.has(parts.rootId)) {
      mindmapSessionSummaryPending.delete(key)
      continue
    }
    if (mindmapSessionSummaryRunning.has(key)) continue
    /* One LLM call at a time across the WHOLE feature: parallel calls on the same provider interfere, so a session-level job must also wait for a running CARD job — and vice versa. Extra ready entries stay pending and the next drain picks them up. */
...
/* 右键会话头 → 总结当前会话: ready sessions are summarized SYNCHRONOUSLY; sessions with missing or in-flight card summaries are force-enqueued (missing only, cooldown bypassed) and parked in the pending set — the drain finishes them in the background and the client sees the result via the next sync. */
export async function summarizeMindmapSession(ctx, persistence, sessionId, config) {
  ...
  /* Not ready: generate the missing card summaries first (force 'missing' retries previously failed turns), then wait for the drain. Scoped to the TARGET session only — a whole-doc scan would bypass the per-sync pacing and the failure cooldown for every other session's turns. */
...
/* Find one turn's question text by owner session + turn/end seq (the card the user right-clicked). */
...
/* User-initiated regeneration: runs SYNCHRONOUSLY so the client can show the result immediately. The failed-cooldown and the background queue are bypassed on purpose — this is an explicit action. */
...
async function regenerateAllBody(ctx, persistence, fresh, config) {
  ...
  /* Auto-regenerate session summaries after the card batch: every session with turns joins the pending set (the drain waits for readiness). */
  for (const session of fresh.sessions ?? []) {
    if (session === null || session === undefined || typeof session.sessionId !== 'string') continue
    const turns = Array.isArray(session.turns) ? session.turns : []
...
export async function listMindmapModels(ctx) {
  /* Optional service via ctx.get (direct ctx.llm access would throw "without inject" on cordis 4 proxies — llm is not in this plugin's inject list). */
  let llm
  try {
    llm = ctx.get('llm')
  } catch (error) {
  ...
  for (const provider of Array.isArray(providers) ? providers : []) {
    ...
    try {
      ...
      /* one provider's catalog is not fatal; log it and keep going (a bogus model entry would render a fake option in the settings picker) */
...
/* Latest durable session title: the last session/title event in its full log (same source as the client's displayTitle). */
async function mindmapTitleOf(ctx, persistence, sessionId) {
  const events = await eventsOf(ctx, persistence, sessionId)
  return mindmapTitleFromEvents(events)
}

/* Backward scan for the last non-empty session/title event, factored out so the doc-build path can derive the root title from the log it ALREADY decoded instead of opening the session a second time. */
...
/* The workspace (cwd) a session belongs to: the live header's cwd first, then the persistence index (non-resident sessions have no live header). */
...
/* Full event log of one session: an attached session's log is read for free via `snapshotEvents()`; everything else goes through a read handle on the persistence backend. `out` (optional) receives the exact fork-inherited event count when the read succeeded. Null when the log is unavailable: callers keep their recorded turns and retry on a later sync. */
...
/* Cached persistence.list(): the harness scan is expensive, so real backend calls are shared in flight and the rows are cached for the TTL window; new fork orphans are still caught immediately by the live-session signal, the doc-write invalidation below, and the periodic full refresh. */
async function mindmapPersistenceList(persistence) {
  ...
  /* In-flight share: two map families poll on independent timers, and a concurrent backend list() would otherwise scan the whole store twice per window. Late callers wait on ONE scan. */
...
/* Parse cache for reconcile: a resident session's full-range snapshot is stable and append-only, so re-parsing is only needed when its identity or length changed — idle family sessions skip the full-log walk on every sync. The TTL bounds same-length in-place edits. */
...
/* A doc path may hold a full document or an alias stub left by a root replacement: { version, aliasTo } pointing at the new root's doc file, so a stale open of the archived root resolves to the current doc instead of building a fresh one. Exported for the GET load path's degraded-reconcile disk fallback. */
export async function readMindmapDocFile(sessionId) {
  /* Follow alias stubs through MULTIPLE hops, cycle-guarded so a corrupt stub loop cannot hang the resolver. */
  let cursor = String(sessionId)
  const seen = new Set()
  while (!seen.has(cursor)) {
    seen.add(cursor)
    const path = mindmapDocPath(cursor)
    /* Probe-path parses are cached by stat fingerprint + TTL (same reasoning as the index cache): sync polls re-read the doc up to twice per cycle and docs hold up to 2 MiB. Only DIRECT reads are cached — a stub-following hop's result depends on OTHER files, so stubs are always re-read. Callers MUTATE the returned doc, so a hit is returned as a fresh structured clone. */
    let cachedEntry
    let stats
    try {
...
/* A mind map whose ROOT session is archived is dead and must stop existing, else reopening the map would resurrect its branches. */
...
/* Delete a stale mind-map file only after re-reading it under the root lock: a root replacement may write an alias between the initial scan and unlink. */
  ...
  if (isValidMindmapDoc(observed)) {
    ...
    return mindmapLock(root, async () => {
      ...
      try {
        ...
        /* Best-effort sweep: an unlink failure must never take down the /index poll or a doc-open request. */
        return false
      }
    })
  }
  if (isPlainObject(observed) && typeof observed.aliasTo === 'string' && observed.aliasTo !== '') {
    ...
    return mindmapLock(aliasTo, async () => {
      ...
      try {
        ...
        /* Best-effort sweep, matching the original cleaner. */
        return false
      }
    })
  }
...
/* Delete every doc whose root session is archived plus dangling alias stubs. Self-healing: any archive is swept by the next index poll or doc access, so it can never be reopened. */
...
/* Resolve the EXISTING doc a session belongs to: the session may be a root or a documented branch of another root (flat scan — docs are few). Null when no doc exists. */
...
/* Resolve the doc a session belongs to, walking UP the fork lineage when the session is not in any doc: an unrecorded fork descendant lands in its ancestor's document instead of becoming a new root. Null only when no ancestor has a doc. */
...
export async function reconcileMindmapDoc(ctx, persistence, doc) {
  ...
  /* Backfill the creation workspace (docs written before the field existed lack it) so a root-node-created top-level session lands in the map's workspace. An EXPLICIT '' (the "ungrouped" choice) is a real selection and must survive. */
  ...
  /* Sessions archived by ANY path are dead: drop them so the map self-heals. The ANCHOR is never dropped here (an archived anchor makes the whole doc dead, swept by the index poll). Children hanging off an archived session are NOT dropped with it — they are live sessions whose only anchor vanished, so each removed session's subtree is re-parented to the removed session's OWN parent card (BFS order, parents before children) to keep the children visible. */
    ...
    if (removed.size > 0) {
      ...
      while (queue.length > 0) {
        ...
        for (const s of sessions) {
          ...
          /* Re-anchor the child to the removed session's OWN parent card — but only when that card is known. A null/undefined parentTurn has no card to hang off, so such children become top-level instead. */
...
/* Merged lineage facts (parent id, seed length, subagent marker) from the live store and the persistence index; live entries win, persisted fill the gaps. */
...
/* The documented chain that OWNS a fork boundary turn: walking UP from the fork's parent, the first session whose forkTurn < boundary owns it. Returns { owner, forkTurn } or undefined on a malformed lineage. */
...
/* Monotonic suffix for adopted-session ids: `Date.now()` alone is millisecond-precision, and two adoption passes in the same millisecond would mint the same id. */
let mindmapAdoptSeq = 0
/* One adoption pass: adopt fork children whose IMMEDIATE parent is already documented. The harness records the fork cut as the child's inheritedEventCount, so the boundary turn is exact even for mid-log forks and already-chatted children; without the cut, the last completed turn is used. Subagent and archived sessions are never adopted. */
async function adoptMindmapOrphanPass(ctx, persistence, doc) {
  ...
  for (const [sessionId, info] of index) {
    ...
    /* Fold the child's own turns (after its fork boundary) with fresh display numbers, like forkBranchAt would. */
...
/* Adopt every harness-created fork child of this doc's family; each pass documents one lineage level, so a fork-of-a-fork converges in a few passes. */
export async function adoptMindmapOrphans(ctx, persistence, doc) {
  ...
  for (let pass = 0; pass < 8; pass += 1) {
    ...
    if (count === 0) break
    adopted = true
    /* The pass cap ran out with this pass STILL adopting: a deeper orphan chain remains. Report it so the sync cache does NOT record adoptClean:true with the unchanged orphan signal, which would strand the deep orphan forever. */
...
/* Reconcile the doc against the current full logs, adopt any harness fork children of its family, and persist. Returns { doc, live }: live is the in-flight turns of each requested family session with an open turn, or [] when none has one. */
/* Normalize the request-dependent live-session selector for the sync cache key: the doc signature alone cannot identify a response — clients may ask for different subsets of the family. */
...
export async function syncMindmapDoc(ctx, persistence, sessionId, liveSessionIds, summaryConfig) {
  ...
  const syncBody = async (fresh) => {
    ...
    /* AI summaries are enqueued on BOTH paths: the cached fast path would otherwise starve backfill. The scan is a cheap read of the in-memory doc and no-ops when the feature is off or nothing lacks a summary. */
    mindmapEnqueueSummaries(ctx, persistence, fresh, summaryConfig, MINDMAP_SUMMARY_ENQUEUE_PER_SYNC)
    /* A sync may also make a pending session summary ready (e.g. a card summary that landed outside the queue path). */
    mindmapDrainPendingSessionSummaries(ctx, persistence)
    ...
    if (cached !== undefined && cached.at + MINDMAP_SYNC_CACHE_TTL_MS > now
      ...
      /* Incremental response: the doc is unchanged, so send doc: null instead of the full document — the client keeps its copy and only applies live/summarizing. The full doc still arrives on every signature change and at least once per TTL. */
    ...
    /* Only bump updatedAt / rewrite the file when the doc ACTUALLY changed: an unchanged cache-miss sync would otherwise rewrite and refresh updatedAt every TTL, re-sorting the sidebar index each poll. The refresh core is fault-isolated and SHARED with the GET load path. */
    ...
    if (refresh.changed) {
      ...
      if (serialized > MINDMAP_DOC_MAX_BYTES) {
        /* Refuse to let the doc grow past the request-side body cap: every later full-doc client write would 413 and lock the map with no shrink path. Keep the disk doc at its last under-limit state, surface a warning, and let the user archive/prune instead of silently losing the new turns. */
    ...
    /* A degraded reconcile/adopt may have PARTIALLY mutated `fresh` in memory; `changed` is false so nothing was written, but the response and the cache must still serve the last good DISK doc — never the half-reconciled object. */
    ...
    /* Settle the cached signature against the just-captured refs so the next poll is already a hit — reusing the probe's parts, so the settle costs no extra index scan. */
    const settled = mindmapSyncSignatureFromParts(ctx, fresh, refs, parts)
    /* Only a CLEAN refresh may seed the cache: a degraded one must not serve the half-reconciled doc as if it were fresh, and a FAILED write must not seed the in-memory doc either (the cache would serve turns that never reached the disk, and a later identical-signature hit would never retry the write). Drop the stale entry so the next sync re-parses and retries. */
    if (refresh.warnings.length === 0 && !syncWriteFailed) {
      /* adoptClean stays false while a deep orphan chain is still being adopted, so the next sync re-runs the adoption. The entry deliberately does NOT carry the doc: the hit path answers doc:null, so storing up to 2 MiB per entry would be pure dead memory. */
  ...
  /* Probe + lock + re-read with automatic re-anchor retry: a root replacement landed between the probe and the lock, so the live doc now lives under a different root. The retry re-acquires only the NEW root's lock, so it never answers exists:false for a doc alive under its new root and can never deadlock. */
...
/* Cheap signature of everything that could change a doc's sync result, plus the family's live event-snapshot references for identity comparison: family logs (snapshotEvents identity + length), new fork orphans (live session id set + persistence index length), and the archived set reference. Split into parts (index-derived, I/O) + from-parts (log-derived, memory) so the sync path computes the parts ONCE per poll and reuses them for the settle. */
async function mindmapSyncSignatureParts(ctx, persistence) {
  let liveIds = ''
  try {
    /* \u0001 separator (not ','): session ids may legally contain commas, and ["a,b","c"] vs ["a","b,c"] would collide into the same signature string, letting the sync cache miss a new fork orphan for up to a TTL. */
...
/* Seed the sync cache at the end of a GET load so the client's first periodic sync is a genuine cache hit. Mirrors the sync settle policy: only a CLEAN refresh whose doc reached the disk may seed; a degraded or unwritten refresh leaves the cache empty so the next sync re-runs the refresh. `flags` = { changed, wrote, adoptIncomplete, warnings }; live is deliberately []. */
...
/* After a root replacement, leave an alias stub at the old root's path so a stale open resolves to the new doc. */
...
/* Persist a client-supplied doc (after a fork, branch removal, or root replacement). With prevSessionId, the new doc and an alias stub for the old root are written in one request, so a root swap never leaves a stale doc. */
export async function writeMindmapDoc(ctx, persistence, sessionId, doc, prevSessionId) {
  ...
  /* Serialized against the periodic sync for the same root, so it can never write a stale read over this doc. A replacement also holds the RETIRED root's lock so the cleaner can never sweep away the stub written here. */
  ...
  return mindmapLocks(lockKeys, async () => {
    ...
    if (prevSessionId === undefined || prevSessionId === null || String(prevSessionId) === String(sessionId)) {
      ...
      if (previous !== null && isValidMindmapDoc(previous)) {
        /* This write's target id has been RETIRED by a root replacement: its path now holds an alias stub that resolves to a DIFFERENT root. Writing the stale full doc onto the stub would resurrect the old root as a second doc file. Refuse instead of corrupting — the caller reloads from the current root. */
        ...
        /* AI-turn summaries are generated Host-side ASYNCHRONOUSLY; a client doc write built from a pre-generation snapshot must not erase them. Fill every incoming turn that has NO summary from the previous doc's turn of the same (sessionId, seq); only MISSING summaries are filled. */
    ...
    if (prevSessionId !== undefined && prevSessionId !== null
      ...
      try {
        ...
        /* A root replacement commits as TWO renamed files (new root doc + old root's alias stub). A stub failure would leave the new file as an orphaned second doc, so roll the new file back (the old doc is still intact and the client's 409/retry path can re-run the replacement). Both root locks are held here, so the rollback cannot race another writer. */
        try {
...
/* Rename ONLY the map's own title (doc.rootTitle) in one Host step. The sidebar previously round-tripped the whole doc, which could clobber a turn a concurrent sync had just folded; a targeted title update leaves the doc untouched and invalidates the sync cache. */
...
/* Remove a doc file (whole mindmap archived). Only the doc's OWN root key may delete it: a branch id or stale alias-stub path resolves to a different rootSessionId, and unlink on such a path silently no-ops while the real doc survives. */
...
/* Index of every doc on disk (sidebar mind-map entries and the branch hider consume it). Archived maps are purged inline in the SAME pass so an archived-root doc disappears within one poll. Fault-isolated like the GET path: a transient fs error on ONE file skips that file, and a directory-level failure degrades to an empty index instead of 500ing the poll. */
export async function indexMindmapDocs(ctx) {
  ...
  for (const name of names) {
    if (!name.endsWith('.json')) continue
    const path = join(mindmapRoot(), name)
    /* Stat-fingerprint reuse: while a file's (ino, size, mtimeMs, ctimeMs) is unchanged since the last poll, serve the cached parse instead of reading the file again. Every doc write is an atomic temp+rename, so the inode swap makes any real change visible on the next poll. */
    let doc
    let stats
    try {
      stats = await stat(path)
      ...
      if (error?.code === 'ENOENT' || error?.code === 'ENOTDIR') {
        /* Vanished between readdir and stat: drop the cached parse; the next readdir will not list it anyway. */

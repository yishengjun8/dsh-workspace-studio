/** Server-side Markdown → HTML rendering for the "open in new window" tab
 *  action on Markdown files. The GUI's own renderer (MarkdownText) is a
 *  client React component that cannot run in the opened tab, so the Host
 *  renders a self-contained document instead. The untrusted-output policy
 *  mirrors MarkdownText: raw HTML renders as literal text, link destinations
 *  pass a protocol allowlist (http/https/mailto), images require absolute
 *  HTTP(S), and disallowed destinations render as plain text. */
...
/* A destination is renderable only when it parses as an absolute URL with an
   allowlisted protocol (MarkdownText parity: relative destinations and
   disallowed schemes render as plain text). */
...
/* Partial renderer merged over marked's defaults via Marked#use: methods
   here run first and only fall back to the default when they return false.
   `this` is the default Renderer instance, so nested children render through
   this.parser (which also routes nested links/images/html back here). */
const SAFE_RENDERER = {
...
/* One shared instance: parse is synchronous and the renderer is stateless,
   so concurrent requests cannot interfere. */
const MARKED = new Marked({ gfm: true })
MARKED.use({ renderer: SAFE_RENDERER })

/* Minimal GitHub-flavored document styling; the opened tab is a unique
   origin and cannot read the GUI theme, so dark mode follows the system. */
...
/* Render a markdown file's bytes into a complete, self-contained HTML
   document (UTF-8). Returns null when the bytes do not decode as the
   detected encoding — the caller then falls back to serving the raw bytes
   (the pre-rendering behavior). */

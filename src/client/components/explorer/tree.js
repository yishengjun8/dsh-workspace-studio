    ...
    const rows = dir.entries.map(entry => {
      ...
      return h(Fragment, { key: entry.path },
        renaming
          ...
          : h(TreeRow, { cut: clipboard?.cut && clipboard?.path === entry.path, depth, entry, expanded: open, onContextMenu: onContextMenu, onDirectory: onDirectory, onFile: onFile, onRename: onRename, selected: selected?.path === entry.path }),

...
export function AppFrame(props) {
  ...
  /* One session's working directory: a preview tab that is handed a path from a
     harness surface (a turn's changed files) resolves it against this, since such
     a path is spelled relative to the SESSION's cwd, not to the workspace root. */
  const sessionCwdOf = useCallback(id => sessionsById[id]?.cwd, [sessionsById])
  useEffect(() => {
...
export function mountStudio(ctx) {
  ...
  /* ctx.layout carries that same selection for the plugins that reach the
     panel face instead of the hook, so it is built from the source above. */
  const layout = new LayoutController(panelInfo)

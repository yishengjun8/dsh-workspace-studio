...
export function createExplorerSettingsStore() {
  return defineStore({
    init: () => ({
      ...
      thinkLines: THINK_LINES_DEFAULT,
      editLines: EDIT_LINES_DEFAULT,
    ...
    actions: {
      ...
      setThinkLines: (draft, value) => { draft.thinkLines = clamp(value, THINK_LINES_MIN, THINK_LINES_MAX) },
      setEditLines: (draft, value) => { draft.editLines = clamp(value, EDIT_LINES_MIN, EDIT_LINES_MAX) },
...
export class LayoutController { constructor(panelInfo){this.panelInfo=panelInfo} attach(actions){this.actions=actions} requireActions(){if(!this.actions)throw new Error('workspace-studio: root store actions are not attached');return this.actions} toggleSidebar(){this.requireActions().toggleSidebar()} openDetails(){this.requireActions().openDetails()} closeDetails(){this.requireActions().closeDetails()} /* The harness ILayout face this plugin's root layout must satisfy: panelInfo is the root store's central-panel selection, which global-panel plugins read through ctx.layout; selectPanel drives the main slot's entryKey; the right-panel verbs are no-ops because this layout has no rightbar track. */ selectPanel(panelId){this.requireActions().selectPanel(panelId)} beginNavigation(){return new AbortController().signal} openRightbar(){} closeRightbar(){} }
...
export function myersDiff(base, mine, alt = false) {
  const N = base.length
  const M = mine.length
  /* Empty-empty is an identity edit: return no changes instead of running the frontier (which would read v[1] out of bounds). */
  if (N === 0 && M === 0) return []
  const max = N + M
  const offset = max
  ...
  for (; d <= max && !found; d += 1) {
    ...
    for (let k = -d; k <= d; k += 2) {
      let x
      /* The tie-break (< vs <=) selects one canonical shortest path when repeated values make the greedy snake ambiguous; `alt` flips it so a merge that clusters poorly can try the other. */
  ...
  let x = N
  let y = M
  // `d` is one past the found end; the trace snapshot for step dd was recorded at the start of iteration dd.
  ...
  changes.reverse()
  // Coalesce adjacent operations (deletion + insertion at the same position = one replacement) so the merge walk sees one change per base span.
  const coalesced = []
...
export function changesTouch(left, right) {
  ...
  // An insertion touches a span only when it lands inside [from, to); at the exclusive end it is disjoint and must merge cleanly.
...
export function sideSliceForSpan(base, side, changes, start, end) {
  ...
  for (const change of changes) {
    ...
    /* sidePos tracks the side index of basePos: the kept run [basePos, change.from) advances it too; compute the kept run length before overwriting basePos. */
...
export function tryMergeWithScripts(base, mine, theirs, mineChanges, theirsChanges, mineText, theirsText) {
  ...
  if (conflicts.length > 0) {
    /* The conflict structure is trustworthy only when each region's side segment matches the real side text mapped through that side's edit script; a non-canonical Myers diff on repeated identical lines can split one replacement so the clustered segment cannot reconstruct one side. If any region fails, fall back to the whole-file conflict. */
    for (const conflict of conflicts) {
      /* A degenerate region (start === end) is an insertion-only clash with no base span to map and no non-canonical collision possible — the structure is exact, skip the check. */
...
export function threeWayMerge(baseText, mineText, theirsText) {
  ...
  // Budget guard first: an oversized file must still fall back to the whole-file dialog, not commit unchecked.
  ...
  /* A non-canonical Myers tie-break can cluster disjoint edits into a false conflict (unsound-cluster); retry once with the alternate canonical shortest path, accepted only when it passes the same round-trip verification. */
...
/* React nodes for one conflict side against the common base: character-level inline diff; oversized regions fall back to line-level marks. */
export function diffRows(baseLines, sideLines) {
  ...
  // Fallback: line-level diff rows for content too large for the character diff.

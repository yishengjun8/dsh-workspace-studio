...
- The Workspace settings page is grouped: **Session Browsing** (the spin speed of the sidebar mind-map entry icon while the map streams, a 0–3x multiplier where larger is faster and the default is 1.5x), **Mind Map Browsing** (hover and selected highlight colors, session-head and end-card accent colors, and the mount-edge curve amount, 0–6x with a 5x default where 0 is a straight line), **File Browsing** (tree row height, search result display, file icon badge colors), **Content Browsing** (per-type highlight presets, save-conflict dialog font size, whether the file browser pane sits left or right of the conversation column (default: left), and file watching with auto-sync (on by default, switchable to notify-only without auto-reload)), and **Conversation Page Settings** (chat font size, Think auto-expand / collapse delay).

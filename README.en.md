...
- Workspace settings page: session browsing (the spin speed of the sidebar mind-map entry icon while the map streams, a 0–3x multiplier where larger is faster and the default is 1.5x), mind-map browsing (hover and selected highlight colors), tree row height, search result display, file icon badge colors, per-type highlight presets, save-conflict dialog font size, chat font size, Think auto-expand / collapse delay, and whether the file browser pane sits left or right of the conversation column (default: left).

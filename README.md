...
- **Browser 端**（`lib/client.js`）提供兼容的 `ctx.layout` 服务，占用根 Slot，继续声明 `sidebar`、`conversation`、`details` 与 `shell.overlay`，并加入文件树、CodeMirror 6 浏览器/编辑器、编辑器上下文行、工作区设置页、`/init` 命令与会话分支导图（预览标签页）。

...
| # | 能力 | 一句话价值 |
|---|---|---|
| 1 | 🗂️ **工作区文件浏览与预览标签页** | 在对话旁边像 IDE 一样浏览工作区：文件树、按会话恢复的预览标签、14 种编码与 Markdown / HTML / 图片渲染视图 |
...
| 💬 **聊天增强** | 会话头部标题区即**会话切换器**（最近更新在前、当前会话高亮、行尾附工作区名、子代理会话带徽标）；Think 条与编辑 / 写入 diff 以**常驻卡片**显示，正文视口行数可调（5–30，默认 10）、可滚动回看、可点击标题收起 |
| 🪟 **新窗口预览** | 右键标签「在新窗口内打开」：Markdown 渲染为文档（GFM，页面无脚本），HTML 原样运行页面脚本，其余文件显示原始文本；链接与图片仅放行 http / https / mailto |
...
- `ctx.sidebarRight.openResource`：把聊天打开文件的路径接管到本插件的预览标签页。
...
PDF 渲染暂未提供；Markdown、HTML、图片、只读文本分页与代码高亮均已覆盖。
...
│   ├── renderers/                        # 渲染器注册表与 Markdown / HTML / 图片 / 分页视图

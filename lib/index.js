...
function reconcileMindmapTurns(parsed, existing, next) {
  ...
  for (const p of parsed) {
    ...
    if (old !== undefined) {
      ...
      const merged = { n: old.n, t: p.t, seq: p.seq, user: p.user }
...
async function mindmapWriteSessionSummary(ctx, persistence, rootId, sessionId, summary) {
  ...
  return mindmapLock(String(probe.rootSessionId), async () => {
    const fresh = await readMindmapDocFile(String(probe.rootSessionId))
    if (fresh === null || !isValidMindmapDoc(fresh) || mindmapDocIsDead(ctx, fresh)) return false
    const session = (fresh.sessions ?? []).find(s => s !== null && s !== undefined && String(s.sessionId) === String(sessionId))
    if (session === undefined) return false
...
async function syncMindmapDoc(ctx, persistence, sessionId, liveSessionIds, summaryConfig) {
  ...
  return mindmapLock(root, async () => {
    ...
    if (cached !== undefined && cached.at + MINDMAP_SYNC_CACHE_TTL_MS > now
      ...
      return { doc: cached.doc, live: Array.isArray(cached.live) ? cached.live : [], summarizing: mindmapSummarizingOf(fresh), sessionSummarizing: mindmapSessionSummarizingOf(fresh) }
    }
    ...
    const before = JSON.stringify({ sessions: fresh.sessions, next: fresh.next, workspaceCwd: fresh.workspaceCwd })
    await reconcileMindmapDoc(ctx, persistence, fresh)
    const adopted = await adoptMindmapOrphans(ctx, persistence, fresh)
    const after = JSON.stringify({ sessions: fresh.sessions, next: fresh.next, workspaceCwd: fresh.workspaceCwd })
    ...
    return { doc: fresh, live, summarizing: mindmapSummarizingOf(fresh), sessionSummarizing: mindmapSessionSummarizingOf(fresh) }
  })
}
    ...
    if (mindmapDocEndpoint) {
      ...
      if (existing !== null) {
        ...
        if (loaded !== null) {
          ...
          mindmapDrainPendingSessionSummaries(ctx, persistence)
          sendJson(req, res, 200, { exists: true, created: false, doc: loaded, summarizing: mindmapSummarizingOf(loaded), sessionSummarizing: mindmapSessionSummarizingOf(loaded) })
          return
        }
      }

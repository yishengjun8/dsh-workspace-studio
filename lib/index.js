...
async function mindmapCwdOf(ctx, persistence, sessionId) {
	...
	if (persistence !== void 0) try {
		...
		for (const row of headers) {
			const header = row === null || row === void 0 ? void 0 : row.header;
			if (header === null || header === void 0 || header.id === void 0) continue;
...
async function coldUsageEvents(persistence, sessionId) {
	...
	try {
		...
		return {
			events: Array.isArray(result) ? result : isPlainObject(result) && Array.isArray(result.events) ? result.events : null,
...
async function refreshUsageIndex(ctx, persistence) {
	...
	if (!isPlainObject(index) || index.version !== USAGE_INDEX_VERSION || !isPlainObject(index.sessions)) index = {
	...
	for (const row of listed) {
		const id = row === null || row === void 0 ? void 0 : row.header?.id;
...
async function handleRequest(ctx, config, trustedHosts, writeQueues, req, res) {
	...
	try {
		...
		const allowed = contextEndpoint ? "POST" : encodingsEndpoint ? "GET, HEAD" : entryEndpoint ? "POST, PATCH" : externalFileEndpoint ? "POST" : fileEndpoint ? "GET, HEAD, PUT" : rawEndpoint ? "GET, HEAD" : fsEndpoint ? "POST" : treeEndpoint ? "GET, HEAD" : searchEndpoint ? "GET, HEAD" : revealEndpoint ? "POST" : draftTreeEndpoint ? "POST" : mindmapDocIndexEndpoint ? "GET, HEAD" : mindmapDocSyncEndpoint ? "POST" : mindmapDocRenameEndpoint ? "POST" : mindmapDocModelsEndpoint ? "GET, HEAD" : mindmapDocRegenerateEndpoint ? "POST" : mindmapDocRegenerateAllEndpoint ? "POST" : mindmapDocSummarizeSessionEndpoint ? "POST" : mindmapForkCleanupEndpoint ? "POST" : updateCheckEndpoint ? "GET, HEAD" : updateDownloadEndpoint ? "POST" : tokenStatsEndpoint ? "GET, HEAD" : mindmapDocEndpoint ? "GET, HEAD, POST, DELETE" : draftEndpoint ? "GET, HEAD, PUT, DELETE" : void 0;

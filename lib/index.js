...
async function buildMindmapDoc(ctx, persistence, sessionId) {
  ...
  try {
    ...
    /* The doc is not on disk; answering created:true would leave the client
       believing the map exists while the next sync answers { exists:false }
       and closes the window. Report "no doc" so the caller answers
       { exists:false } and the overlay closes cleanly. */
    return null
  }
  return doc
}
...
async function purgeArchivedMindmapDocs(ctx) {
  ...
  for (const name of names) {
    ...
    if (isPlainObject(doc) && typeof doc.aliasTo === 'string') {
      /* Follow the alias chain the same way readMindmapDocFile does: a stub
         whose immediate target is ITSELF a stub (a second root replacement) is
         NOT dangling — only a chain that dead-ends is. Unlinking a live
         intermediate stub would break the multi-hop resolution and let a
         future unarchive of the oldest root split the family into a second
         mind map. */
      const resolved = await readMindmapDocFile(doc.aliasTo)
      if (resolved === null) {
        try { await unlink(path); purged += 1 } catch { /* best effort */ }
      }
    }
...
async function syncMindmapDoc(ctx, persistence, sessionId, liveSessionId) {
  ...
  return mindmapLock(root, async () => {
    ...
    let live = null
    if (liveSessionId !== undefined && (String(liveSessionId) === String(fresh.rootSessionId)
      || (fresh.branches ?? []).some(b => b !== null && b !== undefined && String(b?.sessionId) === String(liveSessionId)))) {
    ...
    mindmapSyncCache.set(root, { sig: settled.sig, doc: fresh, live, at: Date.now(), refs: settled.refs })
...
async function indexMindmapDocs(ctx) {
  ...
  for (const name of names) {
    ...
    if (isPlainObject(doc) && typeof doc.aliasTo === 'string') {
      /* Same multi-hop rule as purgeArchivedMindmapDocs: a stub pointing at
         another stub is still alive; only a dead-end chain is dangling. */
      const resolved = await readMindmapDocFile(doc.aliasTo)
      if (resolved === null) {
        try { await unlink(path) } catch { /* best effort */ }
      }

...
function sessionRowHeader(row) {
	const header = isPlainObject(row) ? row.header : void 0;
	return isPlainObject(header) ? header : void 0;
}
...
function sessionRowFingerprint(row) {
	const revision = isPlainObject(row) ? row.revision : void 0;
...
async function refreshUsageIndex(ctx, persistence) {
	...
	for (const entry of entries) {
		...
		if (!(isPlainObject(cached) && cached.unreadable === true && !(Number(cached.at) > 0 && Date.now() - Number(cached.at) < UNREADABLE_RETRY_MS)) && isPlainObject(cached) && cached.rev === identity && (Array.isArray(cached.rows) || cached.unreadable === true)) {

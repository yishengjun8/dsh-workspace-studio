 ...
 * Host watches the file's real path and pushes "changed" events to every
 * listener; because fs.watch is unreliable on some platforms (Windows folder-
 * watch semantics, network drives), a cheap change-check endpoint is kept as a
 * polling fallback. Read-only and scoped to paths already opened in the
...
/** In-memory watch registry: module-scoped plain objects survive a plugin
 * reload (context disposal), so no half-closed watchers are left behind. */
const fileWatchRegistry = {
  ...
  async _refresh(realPath) {
    ...
    if (next === null) {
      // File temporarily gone (editor temp+rename save): keep the last snapshot
      // so a later event re-fire still detects the change.
      return
    }
  ...
  async _onEvent(realPath) {
    const entry = this._watchers.get(realPath)
    if (entry === undefined) return
    // Editors save atomically (temp + rename) or leave an event burst; debounce
    // to the settled file so a half-written preview is never pushed as "new"
    // content. The fallback polling path re-checks stat+hash on its own cadence.
    if (entry.stableTimer !== null) clearTimeout(entry.stableTimer)
...
async function saveFile(workspace, relativePath, config, queues, req, encodingId = 'utf-8') {
  ...
  return serializeWorkspaceMutation(queues, workspace, async () => {
    ...
    /* Re-seed the open-preview baseline so the watcher/poll does not mistake
       this save for an external change (harmless: the tab is clean after commit). */
...
/* Full event log of one session: resident instances are free; others fall back to the persistence backend, which may be absent or fail. */
...
async function handleRequest(ctx, config, trustedHosts, writeQueues, req, res) {
  ...
  try {
    ...
    /* Preview-sync watch endpoints: `control=watch` registers the watcher and
       returns immediately (no streaming); the client polls `check` as a
       fallback. Deliberately request-scoped and read-only. */
    ...
    /* Cheap change check for open preview tabs (client polls when there is no SSE connection or the watcher could not be created). */

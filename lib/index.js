 ...
 * and stored under ~/.dsh-plugin/dsh-workspace-explorer-layout/mindmap/. It is
...
/* Mind-map docs carry every turn's question + answer text, so they routinely
   exceed the generic 4 KiB mutation cap; give them a dedicated 2 MiB bound. */
...
/* First text-bearing block of a content list (reasoning skipped), mirroring
   the client's card label rule. */
function mindmapAnswerOf(blocks) {
...
/* Reverse-parse the FULL event log into completed turns. Each completed turn
   is { t (actual session turn number), seq (turn/end seq), user, answer }.
   The user prompt is the turn's direct human user/message (source.kind ===
   'user'); the answer is the first non-reasoning text block of the turn's
   assistant/message. Compaction replacement copies (surfaceOp != append) are
   skipped so the human transcript (not the model-only surface) is shown. */
function parseMindmapTurns(events) {
  ...
  for (const event of events) {
    ...
    if (event.type === 'turn/start') {
      current = { t: Number(event.data?.turn), seq: undefined, user: '', answer: '' }
      ...
      if (current === null || current.t === undefined) continue
      if (event.surfaceOp !== undefined && event.surfaceOp !== 'append') continue
      const text = mindmapAnswerOf(event.data?.message?.content)
...
function reconcileMindmapTurns(parsed, existing, next) {
  ...
  for (const p of parsed) {
    ...
    if (old !== undefined) {
      out.push({ n: old.n, t: p.t, seq: p.seq, user: p.user, answer: p.answer })
    } else {
      out.push({ n: counter, t: p.t, seq: p.seq, user: p.user, answer: p.answer })
      counter += 1
    }
  }
...
async function buildMindmapDoc(ctx, persistence, sessionId) {
  ...
  try {
    ...
    ctx.logger.warn(`[workspace-explorer-layout] mindmap doc build write failed: ${String(error)}`)
  }
  return doc
}
...
async function readMindmapDocFile(sessionId) {
  const direct = await readJsonFileOrNull(mindmapDocPath(sessionId))
  if (direct !== null) {
    if (isValidMindmapDoc(direct)) return direct
    if (isPlainObject(direct) && typeof direct.aliasTo === 'string' && direct.aliasTo !== String(sessionId)) {
      const aliased = await readJsonFileOrNull(mindmapDocPath(direct.aliasTo))
      if (aliased !== null && isValidMindmapDoc(aliased)) return aliased
    }
  }
  return null
}
        ...
        if (parsed.length <= parentParsed.length
          && parsed.every((turn, index) => {
            ...
            return other !== undefined
              ...
              && String(other.answer ?? '') === String(turn.answer ?? '')
          })) {
...
async function syncMindmapDoc(ctx, persistence, sessionId, liveSessionId) {
  ...
  try {
    ...
    ctx.logger.warn(`[workspace-explorer-layout] mindmap doc sync write failed: ${String(error)}`)
  }
  let live = null
...
async function handleRequest(ctx, config, trustedHosts, writeQueues, req, res) {
  ...
  try {
    ...
    if (!contextEndpoint && !encodingsEndpoint && !entryEndpoint && !externalFileEndpoint && !fileEndpoint && !fsEndpoint && !treeEndpoint && !searchEndpoint && !revealEndpoint && !draftEndpoint && !draftTreeEndpoint && !mindmapDocEndpoint && !mindmapDocIndexEndpoint && !mindmapDocSyncEndpoint) {
      ...
      if (existing !== null) {
        ...
        if (adopted || before !== after) {
          ...
          try {
            ...
            ctx.logger.warn(`[workspace-explorer-layout] mindmap doc load write failed: ${String(error)}`)
          }
        }
      ...
      if (built !== null) {
        ...
        if (built.branches.length > 0) {
          ...
          try {
            ...
            ctx.logger.warn(`[workspace-explorer-layout] mindmap doc conversion adoption write failed: ${String(error)}`)
          }
        }
      }

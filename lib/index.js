...
function normalizeRelativePath(value) {
	...
	for (const part of parts) {
		...
		if (/^(CON|PRN|AUX|NUL|COM[1-9]|LPT[1-9])$/.test(base)) throw new HttpError(400, "invalid-path", "路径段不能使用 Windows 保留名称");
	}
	return parts.join("/");
}
...
async function fileChangeSnapshot(target, previous, maxPreviewBytes) {
	...
	if (previous !== void 0 && previous.mtimeMs === current.mtimeMs && previous.size === current.size && previous?.checkedAt !== void 0 && Date.now() - previous.checkedAt < CHANGE_CHECK_FAST_PATH_TTL_MS) return previous;
...
async function syncMindmapDoc(ctx, persistence, sessionId, liveSessionIds, summaryConfig) {
	...
	return mindmapLock(root, async () => {
		...
		} else if (syncWriteFailed) mindmapSyncCache.delete(root);
		return {
			doc: responseDoc,
			live,

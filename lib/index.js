...
export const Config = z.object({
  ...
  // Cap on bytes the browser may upload for a dragged-in (non-workspace) file.
  // The preview display still truncates to maxPreviewBytes.
...
async function translateToWindowsPath(path) {
  ...
  try {
    ...
    // wslpath missing/failing is a host configuration problem, not a missing
    // path; report a clear reveal failure instead of a misleading path-not-found.
  ...
  for (let lineIndex = 0; lineIndex < lines.length && results.length < cap; lineIndex += 1) {
    ...
    while (results.length < cap && (match = re.exec(text)) !== null) {
      ...
      results.push({
        ...
        // can reveal/select the true match even when the snippet is truncated.
...
async function searchWorkspace(workspace, query, caseSensitive, nameOnly, config) {
  ...
  files.sort()
  directories.sort()
  /* Name-only mode matches each entry's own name — file or directory — and
     reads no file content. Content mode keeps the per-file read below. */
  ...
  /* Incomplete exactly when the scan stopped before visiting every file (a
     file- or match-cap interruption while files remained). Hitting the match
     cap EXACTLY after the last file is complete, so the old
 ...
 * TextDecoder instances decodeBytes uses. Round-tripping through iconv-lite
 * is lossy in the 0x80..0x9F range (its code points differ from the
...
/* ------------------------------------------------------------------------
 * External file-change watching for clean preview tabs.
 *
 * The browsing pane is snapshot-based: the client only re-reads when the
 * active path or the reload token changes.  To pick up edits made by other
 * tools while a file is merely being browsed, the Host watches the file's
 * real path (verified inside the workspace) and pushes "changed" events to
 * every registered client listener.  Because fs.watch is unreliable on some
 * platforms (Windows folder-watch semantics, network drives), each watch
 * also keeps a cheap change-check endpoint that the client can fall back to
 * polling.  Everything here is read-only and scoped to paths the client has
 * already opened in the preview; no directory tree is watched.
 * ---------------------------------------------------------------------- */

/** sha256 of the first maxPreviewBytes bytes (matches readPreview's revision
 * basis, so a reported "no change" is authoritative for what the preview
 * would show; truncated files simply always report change when touched). */
...
/** Cheap "did the file change since the snapshot?" check: stat fields first,
 * hash only when they moved.  Returns the new snapshot (or null when the file
 * is gone). */
...
/** In-memory watch registry. All state is module-scoped so a plugin reload
 * (context disposal) cannot leave half-closed watchers behind; the watcher
 * map stores plain objects that survive disposal. */
const fileWatchRegistry = {
  _watchers: new Map(), // realPath -> { watcher, clients: Set, snapshot, stableTimer }
  _clientListeners: new Map(), // clientId -> Set<listener>
  _clients: new Map(), // clientId -> Set<realPath>
  /** Register a watcher for the real path (client already open on it). */
  add(realPath) {
    const entry = createWatcher(realPath)
    fileWatchRegistry._watchers.set(realPath, entry)
    void entry.refresh()
  },
  /** Drop the watcher and all of its client refs. */
  remove(realPath) {
    const entry = fileWatchRegistry._watchers.get(realPath)
    if (entry === undefined) return
    for (const client of entry.clients) fileWatchRegistry._removeClientRef(client, realPath)
    entry.watcher.close()
    if (entry.stableTimer !== null) clearTimeout(entry.stableTimer)
    fileWatchRegistry._watchers.delete(realPath)
  },
  /** Bind one client to the real path; creates the watcher when needed. */
  addClient(realPath, clientId) {
    const clients = this._clients.get(clientId)
    if (clients !== undefined && clients.has(realPath)) return
    const entry = fileWatchRegistry._watchers.get(realPath)
    if (entry === undefined) {
      fileWatchRegistry.add(realPath)
    }
    fileWatchRegistry._watchers.get(realPath)?.clients.add(clientId)
    if (clients === undefined) fileWatchRegistry._clients.set(clientId, new Set([realPath]))
    else clients.add(realPath)
  },
  /** Drop a whole client (connection close / unsubscribe). */
  removeClient(clientId) {
    const clients = this._clients.get(clientId)
    if (clients === undefined) return
    for (const realPath of clients) this._removeClientRef(clientId, realPath)
    this._clients.delete(clientId)
    this._prune()
  },
  _removeClientRef(clientId, realPath) {
    const clients = this._clients.get(clientId)
    if (clients !== undefined) clients.delete(realPath)
    const entry = this._watchers.get(realPath)
    if (entry !== undefined) entry.clients.delete(clientId)
  },
  _prune() {
    for (const [realPath, entry] of this._watchers) {
      if (entry.clients.size === 0) {
        entry.watcher.close()
        if (entry.stableTimer !== null) clearTimeout(entry.stableTimer)
        this._watchers.delete(realPath)
      }
    }
  },
  async _refresh(realPath) {
    const entry = this._watchers.get(realPath)
    if (entry === undefined || entry.clients.size === 0) return
    const next = await fileChangeSnapshot(realPath, entry.snapshot, this.maxPreviewBytes ?? 1024 * 1024)
    if (next === null) {
      // File temporarily gone (editor save via temp+rename): keep the last
      // snapshot so a later event re-fire still detects the change.
      return
    }
    const changed = entry.snapshot !== undefined
      && (entry.snapshot.mtimeMs !== next.mtimeMs || entry.snapshot.size !== next.size || entry.snapshot.hash !== next.hash)
    entry.snapshot = next
    if (changed) this._notify(realPath, 'changed')
  },
  async _onEvent(realPath) {
    const entry = this._watchers.get(realPath)
    if (entry === undefined) return
    // Editors save atomically (temp file + rename) or leave a burst of
    // events; debounce to the settled file before re-hashing, so a
    // half-written preview is never pushed as the "new" content. The
    // fallback polling path re-checks stat+hash on its own cadence.
    if (entry.stableTimer !== null) clearTimeout(entry.stableTimer)
    entry.stableTimer = setTimeout(() => {
      entry.stableTimer = null
      void this._refresh(realPath)
    }, 220)
  },
  _notify(realPath, kind) {
    for (const [clientId, paths] of this._clients) {
      if (paths.has(realPath)) {
        for (const listener of this._clientListeners.get(clientId) ?? []) {
          listener({ kind, path: realPath })
        }
      }
    }
  },
  ...
  subscribe(clientId, listener) {
    let listeners = this._clientListeners.get(clientId)
    if (listeners === undefined) {
      listeners = new Set()
      this._clientListeners.set(clientId, listeners)
    }
    listeners.add(listener)
  },
  unrefListener(clientId, listener) {
    const listeners = this._clientListeners.get(clientId)
    if (listeners === undefined) return
    listeners.delete(listener)
    if (listeners.size === 0) this._clientListeners.delete(clientId)
  },
}
function createWatcher(realPath) {
  const watcher = fsWatch(realPath, (eventType) => {
    fileWatchRegistry._onEvent(realPath, eventType)
  })
  const entry = {
    watcher,
    clients: new Set(),
    snapshot: undefined,
    stableTimer: null,
    refresh: () => fileWatchRegistry._refresh(realPath),
  }
  fileWatchRegistry._watchers.set(realPath, entry)
  void entry.refresh()
  return entry
}
fileWatchRegistry.maxPreviewBytes = 1024 * 1024
fileWatchRegistry._clientListeners = new Map()

/** Read only the head of a file (never the whole content): stat fields plus a
 * hash of the preview-sized prefix, used for cheap change detection. */
...
/**
 * Preview a non-workspace file uploaded after a drag-and-drop into the preview
 * pane. Browsers never expose a dropped file's absolute path, so the client
 * uploads the raw bytes and this route decodes them like readPreview. Always
 * read-only: there is no known disk location to write back to.
 */
...
/** Serialize every source-workspace mutation through one queue. The coarse
 * workspace-wide lock covers overlapping source paths and target names that
 * are not known until canonical checks run, keeping plugin-originated
 * mutations deterministic and destination allocation race-free. */
...
async function saveFile(workspace, relativePath, config, queues, req, encodingId = 'utf-8') {
  ...
  // so every plugin-originated rename, delete, or save observes one serial history.
  return serializeWorkspaceMutation(queues, workspace, async () => {
    ...
    /* Keep the open-preview change baseline in sync with this write so the
       watcher/poll does not mistake the plugin's own save for an external
       change (the tab is clean after commit anyway; re-seeding is harmless). */
    fileWatchRegistry._refresh(target)
    ...
    try {
      ...
      // Recheck the directory immediately before the path-based rename. This
      // narrows the symlink-swap window and rejects a parent that changed after
      // the temp file was created; a directory-handle rename is unavailable in
      // Node's cross-platform promise API, so this remains a final best-effort
      // fence for hostile local writers.
...
async function deleteEntry(workspace, relativePath, config, queues) {
  ...
  return serializeWorkspaceMutation(queues, workspace, async () => {
    ...
    /* Re-verify the path components immediately before the destructive commit
       (the same final check saveFile/copyEntry perform): a parent swapped to
       an out-of-workspace symlink since the first check would otherwise make
       rm resolve through it and delete outside the workspace. */
    if (await hasSymlinkComponent(root, relativePath)) throw new HttpError(403, 'symlink-write-denied', '拒绝删除符号链接路径')
 ...
 * file OUTSIDE the workspace (~/.dsh-plugin/dsh-workspace-studio/drafts/
 * <workspaceId>/); the source file is never touched until an explicit save,
 * and refreshing the page re-reads the draft. The draft JSON carries the
 * current edit plus the snapshot (base text + base revision) taken when
 * editing began, so restore and the save-time three-way merge need no other
 * storage.
 */
...
function draftWorkspacePart(workspaceId) {
  ...
  // Existing workspace ids are UUIDs. Hash unusual ids rather than letting a
  // future registry implementation turn the draft root into a path join —
  // `.` / `..` pass the allowlist but would resolve one level above the root.
 ...
 * (the first session converted; the "root node" rendered at the top of the
 * map is a VIRTUAL node, not a session) and stored under
 * ~/.dsh-plugin/dsh-workspace-studio/mindmap/. It is the single source of
 * truth for the "导图" conversation view. There is no trunk / main-round
 * concept anymore: the map is a flat list of SESSIONS, each a horizontal
 * chain of question cards, where every session hangs either directly off the
 * virtual root node (parentSessionId null, a "top-level" session, created by
 * clicking the root node) or off a specific question card of another session
 * (a nested fork, parentSessionId + parentTurn). Each turn carries a doc-wide
 * display number `n`, the session's own turn number `t`, and the turn/end
 * `seq` used as the fork boundary. A session records its fork boundary
 * (parent session + turn); the Host re-parses each session's full log during
 * sync so new turns are folded in regardless of the client's conversation
 * window. Version 2 docs (trunk + branches) are normalized to version 3 on
 * read.
 */
...
/* In-memory sync cache keyed by the ROOT session id: the client's periodic
   ...
   re-scan the whole session index on each tick. A cheap signature detects
   "nothing changed" and serves the cached doc; every mutation path (write,
   delete, purge) invalidates the entry, and the TTL forces a periodic full
   sync so in-place log edits the signature cannot see are never missed
   forever. */
...
/* Serialize every mind-map doc mutation per ROOT session: sync (re-parse and
   ...
   stale sync overwrite a just-written doc and re-seed the sync cache with it —
   the "删了又回来" class the cache invalidation was written to prevent. The
   same lock covers rename, delete and the GET handler's reconcile-and-write
   path. Callers re-read the doc INSIDE the lock so a mutation that landed
   between their probe and the lock is picked up, never clobbered. */
...
/* Acquire several per-root locks in sorted order, then run `operation` while
   holding all of them. Multi-key writers (a root replacement touches BOTH the
   new root's doc file and the retired root's alias stub) must use this so the
   cleaner's per-root lock on either path fully serializes against them, and so
   two multi-key writers can never deadlock (sorted acquisition order). */
...
/* A doc on disk may be version 3 (sessions) or an older version 2 (trunk +
   branches); the latter is normalized to v3 on read so nothing downstream
   ever touches the legacy fields. */
...
/* Normalize a persisted doc to the current (v3) shape in place. Version 2
   docs map the trunk (the old root session's turns) onto the first top-level
   session and every branch onto a session hanging off its recorded parent
   card; `id` is kept for backward compatibility with the client's keys. */
function normalizeMindmapDoc(doc) {
  ...
  if (doc.version === 2) {
    ...
    /* The anchor session is always present (a v2 doc always has a root), even
       when its trunk was empty. */
    sessions.push({
      id: `s0`,
...
/* Reverse-parse the FULL event log into completed turns { t (actual session
   turn number), seq (turn/end seq), user }. The prompt is the turn's direct
   human user/message (source.kind === 'user'); the answer text is deliberately
   NOT stored (the cards only ever show the question, so persisting every
   answer would double the doc size for nothing). Compaction replacement copies
   (surfaceOp != append) are skipped so the human transcript (not the
   model-only surface) is shown. */
...
/* The workspace (working directory) a session belongs to: the live header's
   cwd first, then the persistence index (a non-resident session has no live
   header). Returns undefined when unknown. */
...
/* Build a fresh v3 doc for a session that has never been converted: the
   session becomes the doc's first TOP-LEVEL session (hanging off the virtual
   root node) carrying its completed turns. A session with no completed turn
   still converts — the map's root node is the creation hub, and an empty
   session renders as a "等待新问题…" branch. Null only when the session is
   archived (a doc must never be (re)built for an archived session: the log-
   based rebuild would resurrect it). The workspace the map was created in is
   recorded (workspaceCwd, from the anchor session's header) so a top-level
   session later created by clicking the root node lands in the SAME
   workspace instead of wherever the anchor currently lives. */
async function buildMindmapDoc(ctx, persistence, sessionId) {
  ...
  // The caller persists the freshly built document while holding the root lock.
  // Keeping construction side-effect free prevents an unlocked first-write from
  // racing the sync/fork paths.
  return doc
}
...
/* Delete one stale mind-map file only after re-reading it under the relevant
   root lock. A root replacement may write an alias between the initial scan and
   unlink; the locked re-read prevents a cleaner from deleting that new alias. */
  ...
  if (isValidMindmapDoc(observed)) {
    ...
    return mindmapLock(root, async () => {
      ...
      try {
        ...
        /* Best-effort sweep, matching the original cleaner: an unlink failure
           (locked file, AV interference, permissions) must never take down the
           /index poll or a doc-open request; the next sweep retries it. */
        return false
      }
    })
  }
  ...
  for (const name of names) {
    ...
    if (isPlainObject(doc) && typeof doc.aliasTo === 'string') {
      /* Follow the alias chain under its target root lock; only a dead-end
         alias observed again inside that lock may be removed. */
      if (await unlinkStaleMindmapFile(ctx, path, doc)) purged += 1
    }
  }
  return purged
...
/* The doc's tombstone key set (field `deleted`, added by the client's card
   deletion; absent when no card was ever deleted). */
...
/* Reconcile a doc against the CURRENT full logs: re-parse every session's log
   into its own turns (those after its fork boundary), keeping display numbers
   stable. Sessions whose log is currently unavailable keep their recorded
   turns. Mutates the doc (doc.next updated) and returns it. */
async function reconcileMindmapDoc(ctx, persistence, doc) {
  ...
  /* Backfill the recorded creation workspace (documents converted before the
     field existed, incl. v2-normalized docs) so a root-node-created top-level
     session always lands in the map's workspace. */
  ...
  /* Sessions whose session was archived by ANY path (the map toolbar, the
     sidebar, or the harness's own archive) are dead: drop them so the map
     self-heals on the next sync instead of resurrecting archived sessions.
     The ANCHOR is never dropped here: an archived anchor makes the whole doc
     dead (mindmapDocIsDead), swept by the index poll — dropping it first
     would just leave a root-less doc file around. */
...
/* The documented chain that OWNS a fork boundary turn: walking UP from the
   ...
   number owns it — a session's own turns start at forkTurn + 1, every turn it
   inherited has t <= forkTurn, and a top-level session (hanging off the
   virtual root node, forkTurn 0) owns the rest.
...
/* One adoption pass: adopt fork children whose IMMEDIATE parent is already
   documented into the doc as sessions. The harness records the fork cut in
...
/* Reconcile the doc against the current full logs, then adopt any harness
   fork children of its family into it, and persist. Returns { doc, live } —
   live is an array of in-flight turns ({ sessionId, turn, question }) for
   each liveSessionId that is a member of the doc family and has a turn in
   flight (the client shows one streaming card per entry), or [] when no
   session has an open turn / the session has no doc yet (the caller answers
   { exists: false }). */
/* Normalize the request-dependent live-session selector for the sync cache.
   The document signature alone cannot identify a response because two clients
   may ask for different subsets of the same family. */
...
async function syncMindmapDoc(ctx, persistence, sessionId, liveSessionIds) {
  ...
  /* The read-modify-write runs under the per-root lock: a client POST (fork /
     card-delete / truncation) can no longer interleave with the sync's stale
     write-back, and the doc is re-read INSIDE the lock so a write that landed
     between the probe above and the lock acquisition is picked up. */
  return mindmapLock(root, async () => {
    ...
    /* Only bump updatedAt / rewrite the file when the doc ACTUALLY changed:
       a periodic cache-miss sync with unchanged logs would otherwise rewrite
       the file and refresh updatedAt every TTL (30 s), making the doc look
       perpetually updated and re-sorting the sidebar index on each poll. */
    ...
    /* Collect the in-flight turn of each requested doc-family session (the
       client asks for every family session that is currently running). */
    const live = []
...
async function writeMindmapDoc(ctx, persistence, sessionId, doc, prevSessionId) {
  ...
  /* Serialized against the periodic sync for the same root, so the sync can
     never write its stale read over this doc (or over a freshly written alias
     stub during a root replacement). A root replacement also holds the RETIRED
     root's lock: the cleaner re-reads the old path under that lock before
     unlinking, so the alias stub written here can never be swept away between
     this request's write and the cleaner's re-read. */
...
async function renameMindmapDoc(ctx, persistence, sessionId, title) {
  ...
  return mindmapLock(String(doc.rootSessionId), async () => {
    /* Re-read inside the lock: the first read above only picks the queue key;
       writing the stale probe back would drop a turn a concurrent sync folded
...
/* Remove a doc file (whole mindmap archived). Only the doc's OWN root key may
   delete it: a branch id or a stale alias-stub path would resolve to a doc
   whose rootSessionId differs, and unlink on such a path silently no-ops
   (the path does not hold the doc file) while the REAL doc survives on disk. */
async function deleteMindmapDoc(sessionId) {
  ...
  /* Serialized against a concurrent sync for the same root so the sync cannot
     recreate the file right after the unlink. */
...
async function indexMindmapDocs(ctx) {
  ...
  for (const name of names) {
    ...
    if (doc === null) continue
    if (isValidMindmapDoc(doc)) {
      /* An archived map is dead: remove it only after the locked re-read
         confirms that a root replacement did not install an alias in its place. */
    ...
    if (isPlainObject(doc) && typeof doc.aliasTo === 'string') {
      /* Same multi-hop rule as purgeArchivedMindmapDocs, with a locked re-read
         so a concurrent root replacement cannot lose its alias stub. */
      await unlinkStaleMindmapFile(ctx, path, doc)
    }
  }
...
/** Delete one draft: write a tombstone rather than unlinking, so a late PUT for
 * a path that did not yet have a draft is still rejected by the owner
 * generation fence. */
 ...
 * tombstone's only remaining jobs are suppressing restore of a discarded draft
 * and idempotent duplicate deletes — neither needs the file to live forever.
 * Reclaim tombstones older than the retention window whenever a tree operation
 * already holds the full record list, keeping the draft directory bounded
 * without ever touching the fence. */
...
/** Move or delete every staged draft below a relative path, serialized per owner
 * with a generation (tombstones make a late autosave fail even when that path
 * had no draft at the instant of the tree operation). */
...
function validatePromptContextPayload(value, config) {
  ...
  // supported encodings; absent payloads default to UTF-8. Unknown ids throw
  // via encodingById.
...
async function handleRequest(ctx, config, trustedHosts, writeQueues, req, res) {
  ...
  try {
    ...
    const previewSyncEndpoint = url.pathname === `${API_PREFIX}/preview-sync`
    ...
    if (!contextEndpoint && !encodingsEndpoint && !entryEndpoint && !externalFileEndpoint && !fileEndpoint && !fsEndpoint && !treeEndpoint && !searchEndpoint && !revealEndpoint && !draftEndpoint && !draftTreeEndpoint && !previewSyncEndpoint && !mindmapDocEndpoint && !mindmapDocIndexEndpoint && !mindmapDocSyncEndpoint && !mindmapDocRenameEndpoint) {
    ...
    /* Preview-sync watch endpoints (host watch + change-check fallback).
       `control=watch` registers the watcher and returns immediately (no
       streaming); the client polls `check` as a fallback.  Deliberately kept
       request-scoped and read-only. */
    if (previewSyncEndpoint) {
      ...
      if (relativePath === '') throw new HttpError(400, 'invalid-path', '预览同步必须指定文件路径')
      const control = url.searchParams.get('control')
      const clientId = url.searchParams.get('clientId')
      if (control === 'watch' && typeof clientId === 'string' && clientId !== '') {
        const target = await resolveWorkspacePath(await realpath(workspace.path), relativePath)
        const clients = fileWatchRegistry._clients.get(clientId) ?? (fileWatchRegistry._clients.set(clientId, new Set()), fileWatchRegistry._clients.get(clientId))
        if (!clients.has(target)) {
          clients.add(target)
          const entry = fileWatchRegistry._watchers.get(target)
          if (entry === undefined) fileWatchRegistry.add(target)
          else entry.clients.add(clientId)
        }
        sendJson(req, res, 200, { watching: true, workspaceId: String(workspace.id), path: relativePath })
        return
      }
      if (control === 'unwatch' && typeof clientId === 'string' && clientId !== '') {
        fileWatchRegistry.removeClient(clientId)
        sendJson(req, res, 200, { watching: false, workspaceId: String(workspace.id), path: relativePath })
        return
      }
      throw new HttpError(400, 'invalid-preview-sync', '预览同步请求无效')
    }
    ...
    if (mindmapDocSyncEndpoint) {
      ...
      /* Prefer the plural selector, but keep accepting the old singular field
         during rolling upgrades. Legacy callers receive one object-shaped live
         value; new callers receive the array response. */
    ...
    if (mindmapDocEndpoint) {
      ...
      if (existing !== null) {
        /* Fold the latest turns and adopt harness-created fork children so a
           ...
           on updatedAt). Runs under the per-root lock with a fresh read, so a
           concurrent sync folding a turn (or a client write) is never
           clobbered by this path's write-back. */
      ...
      /* First access: serialize the conversion under the session's root lock.
         The second lookup closes the two-first-open race; sync/fork writers for
         a normal root use the same key and cannot be overwritten by this stale
         build snapshot. */
    ...
    /* Cheap change check for open preview tabs (the client polls this when it
       has no SSE connection or the watcher could not be created). */
    if (url.searchParams.get('check') === '1') {
      if (relativePath === '') throw new HttpError(400, 'invalid-path', '变更检查必须指定文件路径')
      const previousRaw = url.searchParams.get('prev')
      const snapshot = await readPreviewHead(workspace, relativePath, config.maxPreviewBytes, undefined)
      let changed = false
      if (snapshot !== null && previousRaw !== null) {
        try {
          const previous = JSON.parse(previousRaw)
          changed = previous?.mtimeMs !== snapshot.mtimeMs
            || previous?.size !== snapshot.size
            || previous?.hash !== snapshot.hash
        } catch { /* malformed prev: treat as changed */ }
      }

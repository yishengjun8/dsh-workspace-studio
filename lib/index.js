...
function encodeText(text, encodingId, withBom = true) {
	...
	if (encodingId === "utf-16le" && withBom) body = Buffer.concat([Buffer.from([255, 254]), body]);
	else if (encodingId === "utf-16be" && withBom) body = Buffer.concat([Buffer.from([254, 255]), body]);
	return body;
}
...
async function mindmapWriteSummary(ctx, persistence, rootId, sessionId, seq, summary) {
	...
	return mindmapLock(String(probe.rootSessionId), async () => {
		...
		if (String(fresh.rootSessionId) !== String(probe.rootSessionId)) return mindmapLocks([String(probe.rootSessionId), String(fresh.rootSessionId)], async () => {
...
async function regenerateAllMindmapSummaries(ctx, persistence, sessionId, config) {
	...
	await mindmapLock(root, async () => {
		...
		if (String(fresh.rootSessionId) !== String(root)) return mindmapLocks([String(root), String(fresh.rootSessionId)], async () => {
			const reRead = await readMindmapDocFile(String(root));
			if (reRead === null || !isValidMindmapDoc(reRead) || mindmapDocIsDead(ctx, reRead)) throw new HttpError(404, "mindmap-not-found", "导图文档不存在");
			if (String(reRead.rootSessionId) !== String(fresh.rootSessionId)) throw new HttpError(404, "mindmap-not-found", "导图文档不存在");
...
async function syncMindmapDoc(ctx, persistence, sessionId, liveSessionIds, summaryConfig) {
	...
	return mindmapLock(root, async () => {
		...
		if (String(fresh.rootSessionId) !== root) return mindmapLocks([root, String(fresh.rootSessionId)], async () => {
...
async function renameMindmapDoc(ctx, persistence, sessionId, title) {
	...
	return mindmapLock(String(doc.rootSessionId), async () => {
		...
		if (fresh === null || !isValidMindmapDoc(fresh) || mindmapDocIsDead(ctx, fresh)) throw new HttpError(404, "mindmap-not-found", "导图文档不存在");
		if (String(fresh.rootSessionId) !== String(doc.rootSessionId)) return mindmapLocks([String(doc.rootSessionId), String(fresh.rootSessionId)], async () => {
		...
		if (mindmapDocEndpoint) {
			...
			if (existing !== null) {
				const loaded = await mindmapLock(String(existing.rootSessionId), async () => {
					...
					if (String(fresh.rootSessionId) !== String(existing.rootSessionId)) return mindmapLocks([String(existing.rootSessionId), String(fresh.rootSessionId)], async () => {

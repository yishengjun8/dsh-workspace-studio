...
async function syncMindmapDoc(ctx, persistence, sessionId, liveSessionIds) {
  ...
  return mindmapLock(root, async () => {
    ...
    const before = JSON.stringify({ trunk: fresh.trunk, branches: fresh.branches, next: fresh.next })
    await reconcileMindmapDoc(ctx, persistence, fresh)
    const adopted = await adoptMindmapOrphans(ctx, persistence, fresh)
    const after = JSON.stringify({ trunk: fresh.trunk, branches: fresh.branches, next: fresh.next })
    ...
    if (liveIds.length > 0) {
      ...
      for (const branch of fresh.branches ?? []) {
        if (branch !== null && branch !== undefined && typeof branch?.sessionId === 'string') family.add(String(branch.sessionId))
      }

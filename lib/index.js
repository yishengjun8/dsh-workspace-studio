...
async function saveFile(workspace, relativePath, config, queues, req, encodingId = 'utf-8') {
  ...
  // In-process route: canonical checks run inside the workspace mutation queue so every
  // plugin-originated rename, delete, or save observes one serial history.
...
async function deleteEntry(workspace, relativePath, config, queues) {
  ...
  return serializeWorkspaceMutation(queues, workspace, async () => {
    ...
    /* Re-verify the path components immediately before the destructive commit
       (same final check saveFile/copyEntry perform): a parent swapped to an
       out-of-workspace symlink since the first check would otherwise make rm
       resolve through it and delete outside the workspace. */
    if (await hasSymlinkComponent(root, relativePath)) throw new HttpError(403, 'symlink-write-denied', '拒绝删除符号链接路径')
...
function draftWorkspacePart(workspaceId) {
  ...
  // `.` / `..` pass the character allowlist but would resolve the join one
  // level above the drafts root, so they are hashed like any other unusual id.
...
/* Serialize every mind-map document mutation per ROOT session: sync (which
   re-parses the doc it read and writes it back) and the client POSTs (fork /
   card-delete truncation / root replacement) otherwise interleave their
   read-modify-write, letting a stale sync overwrite a just-written doc on disk
   and re-seed the sync cache with it — the "删了又回来" class the cache
   invalidation was written to prevent. The same lock covers rename, delete and
   the GET handler's reconcile-and-write path. Callers re-read the doc INSIDE
   the lock so a mutation that landed between their probe and the lock is
   picked up, never clobbered. */
...
async function syncMindmapDoc(ctx, persistence, sessionId, liveSessionId) {
  ...
  return mindmapLock(root, async () => {
    ...
    await adoptMindmapOrphans(ctx, persistence, fresh)
...
async function deleteMindmapDoc(sessionId) {
  ...
  return mindmapLock(sessionId, async () => {

...
async function revealCommandFor(target, directory, platform = process.platform) {
	if (platform === "win32") return {
		file: "explorer.exe",
		args: directory ? [target] : [`/select,${target}`]
	};
...
function draftOperationToken(action, value) {
	return `${action}:${createHash("sha256").update(JSON.stringify(value)).digest("hex")}`;
}
...
async function reconcileMindmapDoc(ctx, persistence, doc) {
	...
	if (archived.size > 0 && (doc.sessions ?? []).some((s) => s !== null && s !== void 0 && String(s?.sessionId) !== String(doc.rootSessionId) && archived.has(String(s?.sessionId)))) doc.sessions = (doc.sessions ?? []).filter((s) => s === null || s === void 0 || String(s?.sessionId) === String(doc.rootSessionId) || !archived.has(String(s?.sessionId)));
	for (const session of doc.sessions ?? []) {
		if (session === null || session === void 0 || typeof session?.sessionId !== "string") continue;
...
async function syncMindmapDoc(ctx, persistence, sessionId, liveSessionIds, summaryConfig) {
	...
	return mindmapLock(root, async () => {
		...
		if (liveIds.length > 0) {
			const family = /* @__PURE__ */ new Set([String(fresh.rootSessionId)]);
			for (const s of fresh.sessions ?? []) if (s !== null && s !== void 0 && typeof s?.sessionId === "string") family.add(String(s.sessionId));
...
async function writeMindmapDoc(ctx, persistence, sessionId, doc, prevSessionId) {
	...
	return mindmapLocks(lockKeys, async () => {
		...
		doc.next = Math.max(Number.isSafeInteger(doc.next) && doc.next > 0 ? doc.next : 0, mindmapNextOf(doc) + 1);
...
async function handleRequest(ctx, config, trustedHosts, writeQueues, req, res) {
	...
	try {
		...
		if (fileEndpoint && (req.method === "GET" || req.method === "HEAD") && url.searchParams.get("check") === "1") {
			...
			let changed = false;
			if (snapshot !== null && previous !== void 0 && previous !== null) changed = previous?.mtimeMs !== snapshot.mtimeMs || previous?.size !== snapshot.size || previous?.hash !== snapshot.hash;
			sendJson(req, res, 200, {
				workspaceId: String(workspace.id),
				path: relativePath,
				changed,
